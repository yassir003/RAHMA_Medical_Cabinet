...
export async function getMutuelles(
  page = 0,
  size = 10
): Promise<PaginatedResponse<Mutuelle>> {
  const q = new URLSearchParams({ page: String(page), size: String(size) });
...
// ---------------------------------------------------------------------------
// Dossier Remboursement (Reports) endpoints
// ---------------------------------------------------------------------------
...
export interface DossierRemboursement {
  ...
  dateCreation: string;
  dateEnvoi?: string;
  statut: "EN_ATTENTE" | "ENVOYE" | "ACCEPTE" | "REJETE" | "REMBOURSE";
  patientId: number;
  patientNom: string;
  patientPrenom: string;
  mutuelleId: number;
...
export async function updateDossierStatut(id: number, statut: string): Promise<DossierRemboursement> {
